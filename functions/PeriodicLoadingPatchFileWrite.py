...
#%%
    ...
    with open(os.path.abspath(os.path.expanduser(dirSaveFile+ '/'+ fileName + '.dat')),'bw') as f_bin:
        ...
        period_bin = struct.pack('<f', period)
        f_bin.write(period_bin)
        nkey_bin = struct.pack('<i', nkey)
        f_bin.write(nkey_bin)
        ...
        for i,key in enumerate(keys):
            ...
            key_bin = struct.pack('<f', key)
            f_bin.write(key_bin)

            if i == nkey:
                for ii, df in enumerate(loads):
                    loads_bin = struct.pack('<' + str(np.size(df[0])) + 'f', *df[0])
                    f_bin.write(loads_bin)
            else:
                for ii, df in enumerate(loads):
                    loads_bin = struct.pack('<'+str(np.size(df[i]))+'f', *df[i])
                    f_bin.write(loads_bin)
