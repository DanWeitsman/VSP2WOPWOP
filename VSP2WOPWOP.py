...
def main():
    ...
    for iter_geom, dataFileName in enumerate(UserIn['dataFileName']):
        ...
        for i in range(UserIn['Nrotor']):
            ...
            if args.load_type:
                loadParams = {'dFx':mat_read['op'+ str(i+1)]['inplane'][args.azimuth,19:],'dFy':np.zeros(np.shape(mat_read['op'+ str(i+1)]['inplane'][args.azimuth,19:])),'dFz':mat_read['op'+ str(i+1)]['outplane'][args.azimuth,19:],'th':[0,0,0]}
                ConstantLoadingPatchFileWrite('op'+ str(i+1) +'_load', loadParams, geomParams['nXsecs'], dirSaveFile)
            else:
                loadParams = {'dFx':mat_read['op'+ str(i+1)]['inplane'][:,19:],'dFy':np.zeros(np.shape(mat_read['op'+ str(i+1)]['inplane'][:,19:])),'dFz':mat_read['op'+ str(i+1)]['outplane'][:,19:],'th':[0,0,0],'phi': np.linspace(0,2*np.pi,np.shape(mat_read['op'+ str(i+1)]['inplane'][:])[0])}
